import noTaskImage from '../assets/no-projects.png'
import Button from './Button.jsx';

export default function NoTaskSelected() {
  return (
    <div className="mt-24 text-center w-2/3">
      <img src={noTaskImage} alt="An empty task list" className="w-16 h-16 object-contain mx-auto"/>
      <h2 className="text-xl font-bold text-stone-500 mt-4 my-4">No Task Selected</h2>
      <p className="text-stone-400 mb-4">Select a task or get started with a new one</p>
      <p className="mt-8">
        <Button>Create new task</Button>
      </p>
    </div>
  );
}
